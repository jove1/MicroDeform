...
from PySide6.QtWidgets import QApplication, QPushButton, QGridLayout, QWidget, QLabel, QDoubleSpinBox, QSpinBox, QMessageBox, QFileDialog, QMainWindow
...
class Window(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        ui.LogXY.toggled.connect( lambda checked: xy.logger.setLevel(logging.DEBUG if checked else logging.WARNING) )
        ui.LogZ.toggled.connect( lambda checked: z.logger.setLevel(logging.DEBUG if checked else logging.WARNING) )
        ui.LogFineZ.toggled.connect( lambda checked: fz.logger.setLevel(logging.DEBUG if checked else logging.WARNING) )
        ui.LogADC.toggled.connect( lambda checked: adc.logger.setLevel(logging.DEBUG if checked else logging.WARNING) )
        ...
        ui.PlotClear.clicked.connect( lambda checked: history.clear() )
        ...
        ui.Load.clicked.connect(   lambda : [ fz.cmd("VEL 1 {}", self.LoadSpeed),   fz.cmd("MVR 1 {}", self.LoadStep if ui.Compression.isChecked() else -self.LoadStep) ])
        ui.Unload.clicked.connect( lambda : [ fz.cmd("VEL 1 {}", self.UnloadSpeed), fz.cmd("MVR 1 {}", -self.UnloadStep if ui.Compression.isChecked() else self.UnloadStep) ])
        ...
        self.adc_data.connect( self._adc_data )
        self.xy_pos.connect( self._xy_pos )
        self.xy_err.connect( self._xy_err )
        self.z_pos.connect( self._z_pos )
        self.fz_pos.connect( self._fz_pos )
        self.fz_volt.connect( self._fz_volt )
        self.fz_err.connect( self._fz_err )
        #self.was_running = False
        #self.fz_status.connect( self._fz_status )

        self.msgbox = QMessageBox(QMessageBox.Warning, "Error", "", parent=self)
        self.msgbox.setWindowModality(Qt.NonModal)

    def __getattr__(self, name):
        return getattr(self.ui, name).value()
    
    def XPlus(self):   xy.cmd("1 VEL {}", self.XSpeed/1000); xy.cmd("1 MVR 12"); self.Xmoving = True
    def XMinus(self):  xy.cmd("1 VEL {}", self.XSpeed/1000); xy.cmd("1 MVR -12"); self.Xmoving = True
    def XStop(self):   
        if self.Xmoving: 
            xy.cmd("1 STP")
            self.Xmoving = False

    def YPlus(self):   xy.cmd("2 VEL {}", self.YSpeed/1000); xy.cmd("2 MVR 12"); self.Ymoving = True
    def YMinus(self):  xy.cmd("2 VEL {}", self.YSpeed/1000); xy.cmd("2 MVR -12"); self.Ymoving = True
    def YStop(self):   
        if self.Ymoving:
            xy.cmd("2 STP")
            self.Ymoving = False

    def ZPlus(self):   z.cmd("vel {}", int(round(self.ZSpeed/z_step_size)) ); z.cmd("rel 5000"); self.Zmoving = True
    def ZMinus(self):  z.cmd("vel {}", int(round(self.ZSpeed/z_step_size)) ); z.cmd("rel -5000"); self.Zmoving = True
    def ZStop(self):
        if self.Zmoving:
            z.cmd("rel 0")
            self.Zmoving = False

    def FZPlus(self):  fz.cmd("VEL 1 {}", self.FZSpeed); fz.cmd("MOV 1 30"); self.FZmoving = True
    def FZMinus(self): fz.cmd("VEL 1 {}", self.FZSpeed); fz.cmd("MOV 1 0"); self.FZmoving = True
    def FZStop(self):
        if self.FZmoving:
            fz.cmd("STP")
            self.FZmoving = False
    ...
    def keyPressEvent(self, ev):
        ...
        key = ev.key()
        if key == Qt.Key_Escape:       self.AllStop()

        mod = ev.modifiers() 
        if (mod & Qt.ShiftModifier) or \
           (mod & Qt.KeypadModifier):
            if key == Qt.Key_Left:     self.XMinus()
            if key == Qt.Key_Right:    self.XPlus()

            if key == Qt.Key_Down:     self.YMinus()
            if key == Qt.Key_Up:       self.YPlus()

            if key == Qt.Key_PageDown: self.ZMinus()
            if key == Qt.Key_PageUp:   self.ZPlus()

            if key == Qt.Key_Home:     self.FZMinus()
            if key == Qt.Key_End:      self.FZPlus()
    ...
    def keyReleaseEvent(self, ev):
        ...
        key = ev.key()
        if key == Qt.Key_Left:     self.XStop()
        if key == Qt.Key_Right:    self.XStop()

        if key == Qt.Key_Down:     self.YStop()
        if key == Qt.Key_Up:       self.YStop()

        if key == Qt.Key_PageDown: self.ZStop()
        if key == Qt.Key_PageUp:   self.ZStop()

        if key == Qt.Key_Home:     self.FZStop()
        if key == Qt.Key_End:      self.FZStop()
    ...
    def timerEvent(self, ev):
        if self.ui.QueryXYposition.isChecked():
            xy.cmd("0 POS ?").callback( self.xy_pos.emit )
        if self.ui.QueryXYerror.isChecked():
            xy.cmd("0 ERR ?").callback( self.xy_err.emit )
        if self.ui.QueryZposition.isChecked():
            z.cmd("?").callback( self.z_pos.emit )
        if self.ui.QueryFineZposition.isChecked():
            fz.cmd("POS?").callback( self.fz_pos.emit )
        if self.ui.QueryFineZvoltage.isChecked():
            fz.cmd("VOL? 2").callback( self.fz_volt.emit )
        if self.ui.QueryFineZerror.isChecked():
            fz.cmd("ERR?").callback( self.fz_err.emit )

        #fz.cmd("WGO?").callback( self.fz_status.emit )
    ...
    def Record(self):
        if self.fh:
            ...
            fname, _ = QFileDialog.getSaveFileName(self, "Save data", filter="Data files (*.dat)")
            if not fname:
                return
...
'zero_load': {self.zero_load},
    ...
    def LoadWaitUnload(self):
        ...
        rate = int( (t1+t2+t3)/cycle_time/wave_memory*1.1 )
        l1 = int(t1/cycle_time/rate)
        l2 = int(t2/cycle_time/rate)
        l3 = int(t3/cycle_time/rate)
        if self.ui.Compression.isChecked():
            fz.cmd("WAV 1 X LIN {} {} {} {} 0 0", l1+l2, self.LoadStep, 0, l1)
            fz.cmd("WAV 1 & LIN {} {} {} {} 0 0", l3, -self.UnloadStep, self.LoadStep, l3)
        else:
            fz.cmd("WAV 1 X LIN {} {} {} {} 0 0", l1+l2, -self.LoadStep, 0, l1)
            fz.cmd("WAV 1 & LIN {} {} {} {} 0 0", l3, self.UnloadStep, -self.LoadStep, l3)
        fz.cmd("WTR 1 {} 1", rate) # straight line interpolation
        fz.cmd("WSL 1 1") # 1st wave table
        fz.cmd("WGC 1 1") # 1 cycle
        offset = float(fz.cmd("POS?").result()[2:])
        fz.cmd("WOS 1 {}", offset) # offset
        fz.cmd("WGO 1 257") # start, final position is the endpoint
    ...
    def _z_pos(self, s):
        m = re.match("pos ([+-]?\d+) target ([+-]?\d+) vel (\d+) lim1 ([01]) lim2 ([01])", s)
        z = int(m[1])*z_step_size
    ...
    def _adc_data(self, data):
        ...
        data[:,0] = position_calib(data[:,0])
        data[:,1] = load_calib(data[:,1])
        ...
        history.update([pos], [load])
        x = np.array(history.x) * adc.N / adc.rate
        self.history_lines[0].setData(x, history.y1)
        self.history_lines[1].setData(x, history.y2)

        self.xy_lines[0].setData(history.y1, history.y2)
        self.xy_lines[1].setData(history.y1[-1:], history.y2[-1:])
...
with hw.XY() as xy, hw.Z() as z, hw.Piezo() as fz, hw.ADC(window.adc_data.emit) as adc:

    xy.logger.setLevel(logging.DEBUG if window.ui.LogXY.isChecked() else logging.WARNING)
    z.logger.setLevel(logging.DEBUG if window.ui.LogZ.isChecked() else logging.WARNING)
    fz.logger.setLevel(logging.DEBUG if window.ui.LogFineZ.isChecked() else logging.WARNING)
    adc.logger.setLevel(logging.DEBUG if window.ui.LogADC.isChecked() else logging.WARNING)
    ...
    cycle_time = 50e-6 # SPA? 1 0x0E000200
    wave_memory = 8192 # SPA? 1 0x13000004

    z_step_size = 2.5
    position_calib = lambda x: x/0.66666666 + 15 # nominal calibration
    load_calib = lambda x: x*3.434329*9.806 # mN
